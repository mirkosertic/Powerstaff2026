...
class PartnerHistoryIT extends AbstractContainerBaseIT {
    ...
    @Autowired
    PartnerHistoryCommandService historyService
    ...
    def setup() {
        historyTypeId = jdbcClient.sql("INSERT INTO historytype (description) VALUES ('IT-History-Typ')")
                .update()
    ...
    def "Historieneintrag anlegen und per findHistoryByPartner lesen"() {
        ...
        def history = historyService.save(newHistory("IT-History erster Eintrag", historyTypeId, partner.id))
    ...
    def "Historieneintrag loeschen entfernt den Eintrag"() {
        ...
        def history = historyService.save(newHistory("IT-History zum Loeschen", historyTypeId, partner.id))

        when:
        historyService.deleteById(history.id)
    ...
    def "mehrere Eintraege werden absteigend nach creation_date sortiert"() {
        ...
        // Insert with explicit dates via JdbcClient to control order
        jdbcClient.sql("""
    ...
    private static PartnerHistory newHistory(String description, Long typeId, Long partnerId) {
        def h = new PartnerHistory()
